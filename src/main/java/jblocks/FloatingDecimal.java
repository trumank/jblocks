...
class FDBigInt {
    ...
    mult( FDBigInt other ){
        ...
        for( i = 0; i < nWords; i++ ){
            long v = data[i] & 0xffffffffL; // UNSIGNED CONVERSION
            long p = 0L;
            int j;
    ...
    longValue(){
        ...
        assert nWords > 0 : nWords; // longValue confused

        if (nWords == 1)
            return (data[0]&0xffffffffL);

        assert dataInRangeIsZero(2, nWords, this); // value too big
